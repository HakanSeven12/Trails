...
class TaskLandXMLImporter:

    def __init__(self):
        """
        Constructor
        """
        self.form = FreeCADGui.PySideUic.loadUi(ui_path + '/import_landxml_panel.ui')
    ...
    def examine_file(self):
        ...
        subpanel = FreeCADGui.PySideUic.loadUi(ui_path + filename, None)
        self.subtask = landxml_subtask.LandXMLSubtask(subpanel, filepath)
    ...
    def accept(self):
        ...
        for name, s in data['PointClusters'].items():
            ...
            make_cluster.create(points, name)

        for name, s in data['Surfaces'].items():
            points = list(s['Points'].values())
            surf = make_terrain.create(points, name)
