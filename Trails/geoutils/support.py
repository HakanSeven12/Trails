...
def get_ortho(vector, rot):
    ...
    if not isinstance(result, FreeCAD.Vector):
        return None

    return FreeCAD.Vector(result.y, -result.x, 0.0).normalize().multiply(rot)
...
def get_bearing(vector, reference=C.UP):
    ...
    Vector is a list of coordinates or an FreeCAD.Vector
    """
    ...
    if not isinstance(vector, FreeCAD.Vector):
        return None
...
def within_tolerance(lhs, rhs=None, tolerance=None):
    ...
    if not all([
            isinstance(_i, (FreeCAD.Vector, list, float, int)) for _i in item_list
        ]):
...
def vector_ortho(vector):
    ...
    for vec in vec_list:
        result.append(FreeCAD.Vector(-vec.y, vec.x, 0.0))
...
def vector_from_angle(angle):
    ...
    return FreeCAD.Vector(math.sin(_angle), math.cos(_angle), 0.0)
